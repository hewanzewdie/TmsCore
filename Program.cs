...
string[] allCourses = [.. backendCourses, .. frontendCourses, "Cloud Native DevOps"];
...
Student[] students2 = await Task.WhenAll(studentTasks);
Course[] courses = await Task.WhenAll(courseTasks);

Console.WriteLine($"\nLoaded {students2.Length} students and {courses.Length} courses in {sw.ElapsedMilliseconds}ms");
...
decimal classAverage = students2.Length > 0
? students2.Average(s => s.GPA)
: 0m;
Console.WriteLine("\n========== ENROLLMENT SUMMARY ==========");
Console.WriteLine($"Total students loaded: {students2.Length}");
