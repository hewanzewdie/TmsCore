using System;

public class EnrollmentService
{
    public EnrollmentRecord ProcessRegistration(Student? student, Course? course)
    {
        if (student is null)
        {
            throw new ArgumentNullException(nameof(student), "Student data cannot be null.");
        }

        if (course is null)
        {
            throw new ArgumentNullException(nameof(course), "Course data cannot be null.");
        }

        if (course.Capacity <= 0)
        {
            throw new InvalidOperationException($"Cannot register. The course '{course.Title}' has no available capacity.");
        }

        string standing = student.GPA switch
        {
            >= 3.5m => "Honors",
            >= 2.5m => "Good Standing",
            _       => "Academic Warning" 
        };

        Console.WriteLine($"{student.Name} is in {standing}.");
        
        return new EnrollmentRecord(student.Id, course.Code, DateTime.UtcNow);
    }
}